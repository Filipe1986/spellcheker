...
public class Demerau {
	...
	public Demerau(String string1, String string2) {
		if ((string1.length() > 0 || !string1.isEmpty()) || (string2.length() > 0 || !string2.isEmpty())) {
	...
	private void setupMatrix() {
		int cost = -1;
		int delecao, substituicao, insercao;
		...
		for (int i = 1; i <= string1.length(); i++) {
			for (int j = 1; j <= string2.length(); j++) {
				...
				substituicao = matriz[i - 1][j - 1] + cost;
				...
				if ((i > 1) && (j > 1) && (string1.charAt(i - 1) == string2.charAt(j - 2))
					...
					matriz[i][j] = minimo(matriz[i][j], matriz[i - 2][j - 2] + cost);
				}
			}
		}
	...
	private void displayMatrix() {
		...
		for (int y = 0; y <= string2.length(); y++) {
			if (y - 1 < 0)
				...
				System.out.print(string2.charAt(y - 1));
	...
	private int minimo(int d, int i, int s) {
	...
	private int minimo(int d, int t) {
		int valorMaximo = Integer.MAX_VALUE;
	...
	public int getDHSimilarity() {
		...
		for (int i = 1; i < string1.length(); i++) {
			...
			for (int j = 1; j < string2.length(); j++) {
				...
				int i1 = DA[string2.indexOf(string2.charAt(j - 1))];
				int j1 = db;
			...
			DA[string1.indexOf(string1.charAt(i - 1))] = i;
		}
